...
/*   Updated: 2021/07/21 23:32:22 by CWatcher         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char *argv[], char *envp[])
{
	...
	i = argc - 1;
	fd_out = open(argv[i], O_CREAT | O_WRONLY | O_TRUNC,
			S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
	if (fd_out < 0)
		exit_me(ft_strjoin("Failed to open:", argv[i]));
	while (--i > 2)
	{
		if (pipe(pipe_fds) < 0)
			exit_me(ft_strjoin("Failed to pipe() on:", argv[i]));
		fork_cmd(argv[i], envp, pipe_fds[0], fd_out);
		fd_out = pipe_fds[1];
	}
	fork_cmd(argv[2], envp, fd_in, fd_out);
	while (argc-- > 3)
		wait(NULL);
}
